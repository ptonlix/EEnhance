    ...
    def configure(self, config: Dict[str, Any]) -> None:
        ...
        for key, value in config.items():
            if (
                ...
                and isinstance(getattr(self, key), NestedConfig)
            ):
...
def load_conversation_config(
    config_conversation: Optional[Dict[str, Any]] = None
) -> ConversationConfig:
    """
...
def main() -> None:
    ...
    try:
        ...
        print(
            f"NON_EXISTENT_KEY: {custom_config_instance.get('NON_EXISTENT_KEY', 'Default Value')}"
        )
