...
class LongFormContentGenerator:
    ...
    def enhance_prompt_params(
        ...
        COMMON_INSTRUCTIONS = """
            CONTEXT中给出了之前的播客对话内容。
            请自然地延续对话。跟进上一个观点/问题,避免重复已讨论的话题!
            因此,过渡要自然流畅。避免突兀的转换。
            确保第一个发言者与上一个发言者不同。查看CONTEXT中的最后一个标签来确定上一个发言者。
            如果CONTEXT中最后一个标签是<Person1>,那么现在第一个发言者应该是<Person2>。
            如果CONTEXT中最后一个标签是<Person2>,那么现在第一个发言者应该是<Person1>。
            这是一场实时对话,中间没有任何中断。
            因此,避免使用"我们稍后再讨论"或"好的,让我们继续之前的话题"之类的表述。
        """
        ...
        if part_idx == 0:
            ...
            请以问候听众开场: 欢迎收听{enhanced_params["podcast_name"]} - {enhanced_params["podcast_tagline"]}。
            您正在生成长篇播客对话的开场部分。
            暂时不要涉及具体话题,只需介绍自己和主题。请遵循以下指南:
            """
            ...
            您正在生成长篇播客对话的最后部分。
            {COMMON_INSTRUCTIONS}
            对于这部分,请讨论以下输入内容,然后以播客对话的形式做出总结发言,并以Person1的告别语结束对话,遵循以下指南:
            """
        else:
            enhanced_params[
                "instruction"
            ] = f"""
            您正在生成{total_parts}部分中的第{part_idx+1}部分播客对话。
            {COMMON_INSTRUCTIONS}
            对于这部分,请以播客对话的形式讨论以下输入内容,遵循以下指南:
            """
